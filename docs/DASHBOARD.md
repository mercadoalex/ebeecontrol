...
│  └──────────┘  └────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ 🤖 Gemini AI Forensic Reports (Table with Gist links)    │  │
│  └──────────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
│  🧠 Autonomous ML Decisions                                     │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ ML-Driven Response Actions Log (Table)                    │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────┐  ┌────────────────────────────────┐  │
│  │ Actions by Threat    │  │ Success Rate                   │  │
│  │ Level (Bar)          │  │ (Single Value %)               │  │
│  └──────────────────────┘  └────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
...
#### Tile: 🤖 Gemini AI Forensic Reports (Table)

**Purpose:** Links to full AI-generated forensic analysis reports published as GitHub Gists for each incident.

**Columns:** Timestamp, Report ID, Threat Classification, Affected Pod, Namespace, Gist URL

**What it tells you:**
- Every forensic report generated by the Gemini AI analysis engine
- The threat level and affected infrastructure for each report
- A clickable Gist URL linking to the full forensic analysis (attacker intent, timeline, recommended follow-up actions)
- Reports showing "pending..." in the Gist URL column were generated before Gist publishing was configured

**How it works:**
1. When a honeytoken is accessed, the system generates a structured forensic report
2. Gemini AI analyzes the incident context (process, pod, anomaly score, response actions)
3. The full analysis is published as a GitHub Gist for easy sharing and review
4. The Gist URL is stored alongside the report metadata in Dynatrace logs

**DQL Query:**
```
fetch logs
| filter contains(content, "reportId") and contains(content, "reportContent")
| parse content, "JSON:p"
| sort timestamp desc
| limit 5
| fields timestamp, p[reportId], p[threatClassification], p[affectedPodId], p[namespace], gistUrl = if(isNull(p[gistUrl]), "pending...", else: p[gistUrl])
```

**Prerequisites for Gist URLs:**
- `GITHUB_GIST_TOKEN` environment variable must be set
- Token requires `gist` scope permission

---

### Section 6: 🧠 Autonomous ML Decisions

_"What decisions is the ML making? How effective are they?"_

#### Tile: ⚙️ ML-Driven Response Actions Log (Table)

**Purpose:** Detailed audit trail of every autonomous decision made by the ML-driven response engine.

**Columns:** Timestamp, Action ID, Action Type, Target, Triggering Classification, Outcome

**What it tells you:**
- Every autonomous containment decision with full traceability
- Which threat classification triggered each action
- The specific target (pod, IP, namespace) of each response
...
```
fetch logs
| filter contains(content, "actionId") and contains(content, "actionType") and contains(content, "outcome")
| parse content, "JSON:p"
| sort timestamp desc
| limit 20
| fields timestamp, p[actionId], p[actionType], p[target], p[triggeringClassification], p[outcome]
```

---

#### Tile: 🎯 Decision Accuracy: Actions by Threat Level (Bar Chart)

**Purpose:** Visualizes how the ML model's response decisions correlate with threat severity and success rates.

**What it tells you:**
- Whether high-severity threats are getting appropriate containment responses
- The success/failure ratio at each threat level
- If the model is over-responding to low threats or under-responding to critical ones
- Patterns that suggest the model needs retraining
...
| filter contains(content, "actionId") and contains(content, "actionType") and contains(content, "outcome")
| parse content, "JSON:p"
| summarize count=count(), by:{p[triggeringClassification], p[outcome]}
```

---

#### Tile: 📈 Autonomous Response Success Rate (Single Value)

**Purpose:** Overall success percentage of all ML-driven autonomous actions.

**What it tells you:**
- The system's overall containment effectiveness as a single metric
- Color-coded: red (<70%), yellow (70-90%), green (>90%)
- A dropping success rate indicates RBAC issues, network problems, or model drift
...
| filter contains(content, "actionId") and contains(content, "actionType") and contains(content, "outcome")
| parse content, "JSON:p"
| summarize total=count(), successes=countIf(p[outcome] == "success")
| fields successRate = 100.0 * toDouble(successes) / toDouble(total)
```
